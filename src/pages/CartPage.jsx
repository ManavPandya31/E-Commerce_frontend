...
export default function CartPage({cartItems,setCartItems,cartCount,setCartCount,}) {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:3131/api/cart/readAllItems",config,);
      console.log("Response From Get Cart Api :-", response);
  ...
  const fetchAddresses = async () => {
    try {
      ...
      const allAddresses = res.data.data.addresses || [];
      setAddresses(allAddresses);

      if (allAddresses.length > 0) {
        setSelectedAddressId(allAddresses[allAddresses.length - 1]._id);
      }
  ...
  const decreaseQty = async (item) => {
    ...
    try {
      dispatch(showLoader());
      const res = await axios.put("http://localhost:3131/api/cart/updateCartItems",
        {
          productId: item.product._id,
          quantity: item.quantity - 1,
        },config);
        console.log("Descrease Quantity Response :-",res);
  ...
  return (
    <div>
      ...
      {showAddressModal && (
        <div className="modal-backdrop">
          <div
            className={`modal-content cart-address-modal ${
